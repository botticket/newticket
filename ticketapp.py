...
def handle_message(event):
    ...
    try:
        if 'check' in text_from_user:
            ...
            def goldcheck():
                ...
                if float(gspot) >= float(IQXGL):
                    ...
                    word_to_reply2 = text1 + 'Short' + text3
        ...
        elif 'Hello' in text_from_user:
            
            import ticket_year as tiy

            try:
                tiy.update1()
            except:
                text_list = [
                    'วันนี้ {} สามารถพิมพ์ชื่อหุ้นได้เลยนะคะ'.format(disname),
                    'วันนี้ {} ลองหาข้อมูลหุ้นก่อนนะคะ'.format(disname),
        ...
        elif 'Morning' in text_from_user:
...
def RegisRichmenu(event):
    userid = event.source.user_id
    disname = line_bot_api.get_profile(user_id=userid).display_name
    line_bot_api.link_rich_menu_to_user(userid,'richmenu-67c6b3e7abe8b1945a3d3b652d3473cc')

    button_1 = QuickReplyButton(action=MessageAction(lable='check',text='check'))
    button_2 = QuickReplyButton(action=MessageAction(lable='set',text='set'))
    button_3 = QuickReplyButton(action=MessageAction(lable='tfex',text='tfex'))
    button_4 = QuickReplyButton(action=MessageAction(lable='Hello',text='Hello'))
    button_5 = QuickReplyButton(action=MessageAction(lable='Hi',text='Hi'))
    button_6 = QuickReplyButton(action=MessageAction(lable='Morning',text='Morning'))
